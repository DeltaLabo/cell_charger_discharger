...
void scaling() /// This function performs the folowing tasks:
{
    ...
    log_data.temperature = (uint16_t) (kp * 1000000.0);      // WHILE NOT IN SERVICE
    //log_data.temperature = (uint16_t) (test_configuration.order_of_states[counter_state + 2]);
    qavg += (float)iavg / 3600.0; /// <li> Perform the discrete integration of #iavg over one second and accumulate in #qavg 

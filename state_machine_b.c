...
void fCHARGE()
{    
    if ( ( ( (uint16_t) ( ( ( (float)iavg * 2.5 * 5000.0 ) / 4096.0 ) + 0.5 ) < basic_configuration.end_of_charge ) && ( basic_configuration.version == 0x01 ) ) || ( ( ( ((uint16_t) ( ( ( (float)vavg * 5000.0 ) / 4096.0 ) + 0.5 )) < (vmax - basic_configuration.end_of_charge) ) || (timeout < second) ) && ( basic_configuration.version == 0x02 ) ) ) /// * If #vavg is below #EOD_voltage
    {
        if (second > 5)
        {
...
void converter_settings()
{
    ...
    switch(state)
    {
        ...
        case DC_res: /// If the current state is #DC_RES
            iref = (uint16_t) ( ( ( capacity * 4096.0 ) / (5000 * 2.5 * 5) ) + 0.5 ); /// * The current setpoint, #iref is defined as <tt> 0.2 * capacity </tt>

...
def eventBasic():
    try:
        ...
        options = ["Li-Ion", "Ni-MH"]  # Opciones del SelectBox
        ...
        labels = ["Const. Voltage (mV):", "Const. Current Char (mA):", "Const. Current Disc (mA):",
                  "Capacity (mAh):", "end-of-charge (mA or mV):", "end-of-precharge (mA or mV):",
                  "end-of-discharge (mV):", "end-of-postdischarge (mA or mV):"]
        
        default = [3700, 1750, 1750, 3500, 400, 1750, 2500, 1750]
...
def eventConverter():
    try:
        ...
        labels = ["CVKp (xE-6):", "CVKi (xE-6):", "CVKd (xE-3):", 
                  "CVKp Charge (xE-6):", "CVKi Discharge (xE-6):"
                  "CVKp Charge (xE-6):", "CVKi Discharge (xE-6):"]
...
def clear_data():
    global cell_count, repetition_counter, state
    cell_count = 0
    repetition_counter = 0
    state = 0
    elapsed_time.clear()
...
def process_serial_data(packet):
    global cell_count, repetition_counter, state, current_state, csv_file_path
    header = packet[:1]
    tail = packet[16:17]
    if (header == b'\xdd'):
        ...
        currentM.append(round(cmeas * 1000, 3))
...
def apply_basic_config(selectbox, entries, labelSended):
    ...
    #           "DD5A 03 11 01 740E D606 D606 AC0D 9001 D606 C409 D606 1E06 77"
...
MainFrame = customtkinter.CTkScrollableFrame(root, label_text="Calibration")
...
ButtonFrame.grid(row=1, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")
...
########################## GRAPHS #############################
# Read variable monitoring graph
varGraph = Figure(figsize=(6, 4), dpi=100)
axGraph = varGraph.add_subplot(111)
axGraph.set_xlabel('Time (s)')
canvasVar = FigureCanvasTkAgg(varGraph, master=GraphFrame)
canvasVar.get_tk_widget().grid(row = 0, column = 0, padx = 2, pady = 2)

# Real variable monitoring graph
varGraphReal = Figure(figsize=(6, 4), dpi=100)
axGraphReal = varGraphReal.add_subplot(111)
axGraphReal.set_xlabel('Time (s)')
canvasVarReal = FigureCanvasTkAgg(varGraphReal, master=GraphFrame)
canvasVarReal.get_tk_widget().grid(row = 0, column = 1, padx = 2, pady = 2)

# Configurar la comunicación serial
ser = serial.Serial('COM12', 57600, timeout=1)  # Ajusta el puerto y la velocidad según sea necesario

# Botones
buttonBasic = customtkinter.CTkButton(text="BASIC CONFIG", command=eventBasic, master=ButtonFrame, height=50)
buttonBasic.grid(row = 0, column = 0, padx = 2, pady = 2, sticky="nsew")

buttonTest = customtkinter.CTkButton(text="TEST CONFIG", command=eventTest, master=ButtonFrame, height=50)
buttonTest.grid(row = 0, column = 1, padx = 2, pady = 2, sticky="nsew")

buttonConverter = customtkinter.CTkButton(text="CONVERTER CONFG", command=eventConverter, master=ButtonFrame, height=50)
buttonConverter.grid(row = 0, column = 2, padx = 2, pady = 2, sticky="nsew")

buttonStart = customtkinter.CTkButton(text="START", command=eventStart, master=ButtonFrame, height=50)
buttonStart.grid(row = 0, column = 3, padx = 2, pady = 2, sticky="nsew")

buttonNextS = customtkinter.CTkButton(text="NEXT STATE", command=eventNextS, master=ButtonFrame, height=50)
buttonNextS.grid(row = 0, column = 4, padx = 2, pady = 2, sticky="nsew")

buttonNextC = customtkinter.CTkButton(text="NEXT CELL", command=eventNextC, master=ButtonFrame, height=50)
buttonNextC.grid(row = 0, column = 5, padx = 2, pady = 2, sticky="nsew")

buttonReset = customtkinter.CTkButton(text="RESET", command=eventReset, master=ButtonFrame, height=50)
buttonReset.grid(row = 0, column = 6, padx = 2, pady = 2, sticky="nsew")

buttonBack = customtkinter.CTkButton(text="BACK", command=eventBack, master=ButtonFrame, height=50, fg_color="#E8664A", text_color="black", font=("Arial", 12, "bold"))
buttonBack.grid(row = 0, column = 7, padx = 2, pady = 2, sticky="nsew")

# Labels frame
data_frame = customtkinter.CTkFrame(master=MainFrame)
data_frame.grid(row=0, column=2, padx=10, pady=10)

# Labels
cell_label = customtkinter.CTkLabel(data_frame, text="Cell counter: ")
cell_label.grid(row = 1, column = 0, padx = 2, pady = 0, sticky="w")
cell_display = customtkinter.CTkEntry(data_frame, justify="left")
cell_display.grid(row=2, column=0, padx=10, pady=2)

rep_label = customtkinter.CTkLabel(data_frame, text="Repetition counter: ")
rep_label.grid(row = 3, column = 0, padx = 2, pady = 0, sticky="w")
rep_display = customtkinter.CTkEntry(data_frame, justify="left")
...
state_display = customtkinter.CTkEntry(data_frame, justify="left")
state_display.grid(row=6, column=0, padx=10, pady=2)

time_label = customtkinter.CTkLabel(data_frame, text="Elapsed time(s): ")
time_label.grid(row = 7, column = 0, padx = 2, pady = 0, sticky="w")
time_display = customtkinter.CTkEntry(data_frame, justify="left")
time_display.grid(row=8, column=0, padx=10, pady=2)

voltage_label = customtkinter.CTkLabel(data_frame, text="Voltage: ")
voltage_label.grid(row = 9, column = 0, padx = 2, pady = 0, sticky="w")
voltage_display = customtkinter.CTkEntry(data_frame, justify="left")
voltage_display.grid(row=10, column=0, padx=10, pady=2)

current_label = customtkinter.CTkLabel(data_frame, text="Current: ")
current_label.grid(row = 11, column = 0, padx = 2, pady = 0, sticky="w")
current_display = customtkinter.CTkEntry(data_frame, justify="left")
current_display.grid(row=12, column=0, padx=10, pady=2)

cap_label = customtkinter.CTkLabel(data_frame, text="Capacity: ")
cap_label.grid(row = 13, column = 0, padx = 2, pady = 0, sticky="w")
cap_display = customtkinter.CTkEntry(data_frame, justify="left")
cap_display.grid(row=14, column=0, padx=10, pady=2)

dc_label = customtkinter.CTkLabel(data_frame, text="Duty cicle: ")
dc_label.grid(row = 15, column = 0, padx = 2, pady = 0, sticky="w")
dc_display = customtkinter.CTkEntry(data_frame, justify="left")
